...
export default class ContactAddForm extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.state.name !== prevState.name || this.state.phone !== prevState.phone) { //Avoid inf loop
      this.validateForm()
    }
  }
  ...
  handlePhoneChange = phone => {
    if (+phone >= 0 && phone.length <= 10) { //cast to a number
      this.setState({phone})
  ...
  validateForm = () => {
    ...
    if (+this.state.phone >= 0 && this.state.phone.length === 10 && names.length >= 2 && !names.includes('')) {
